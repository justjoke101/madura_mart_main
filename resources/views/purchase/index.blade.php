                        ...
                        @forelse($purchases as $index => $purchase)
                            <tr>
                                ...
                                <td class="align-middle text-center">
                                    ...
                                    {{-- ⭐ JELASKAN 7: TOMBOL EDIT (Memerlukan Password Atasan)
                                        Saat diklik, memanggil fungsi JavaScript askPasswordForEdit()
                                        yang menampilkan modal password. --}}
                                    ...
                                    {{-- ⭐ JELASKAN 8: TOMBOL DELETE (Memerlukan Password Atasan)
                                        Saat diklik, memanggil fungsi JavaScript askPasswordForDelete()
                                        Form ini menggunakan method DELETE untuk menghapus data. --}}
...
<script>
    ...
    /**
     * ⭐ JELASKAN 9: FUNGSI JAVASCRIPT askPasswordForEdit()
     * =============================================
     * Dipanggil saat user klik tombol EDIT.
     *
     * ALUR:
     * 1. Tampilkan popup SweetAlert dengan input password + icon mata
     * 2. User isi password → kirim ke server via AJAX (fetch)
     * 3. Server cek di PurchaseController@checkBossPassword
     * 4. Jika password BENAR → popup "Nice!" → redirect ke form edit
     * 5. Jika password SALAH → popup error "Wrong password!"
     */
    function askPasswordForEdit(event, id) {
        // Cegah link default (href="#")
        event.preventDefault();

        // Tampilkan modal input password menggunakan SweetAlert (dengan tombol show/hide)
        Swal.fire({
            title: 'Password required!',                    // Judul modal
            text: 'Write your boss\'s password:',           // Deskripsi
            // Gunakan HTML custom supaya bisa tambah icon mata (show/hide password)
            html: `
                <div style="position:relative; margin-top:10px;">
                    ...
                    <span onclick="toggleSwalPassword()" id="swal-eye-btn"
            ...
            focusConfirm: false,
            // Ambil value dari custom input saat user klik OK
            preConfirm: () => {
    ...
    /**
     * ⭐ JELASKAN 10: FUNGSI JAVASCRIPT askPasswordForDelete()
     * =============================================
     ...
     * 3. Jika password BENAR → popup konfirmasi "Are you sure?" → hapus
     * 4. Jika password SALAH → popup error
     */
    function askPasswordForDelete(event, id, note) {
        // Cegah link default
        event.preventDefault();

        // Tampilkan modal input password (dengan tombol show/hide)
        Swal.fire({
    ...
    /**
     * Fungsi Toggle Show/Hide Password di modal SweetAlert
     * Saat icon mata diklik, input berubah dari password → text (terlihat) dan sebaliknya
     */
    function toggleSwalPassword() {
        ...
        if (input.type === 'password') {
            ...
            icon.classList.add('fa-eye-slash');          // Ganti icon jadi mata tertutup
        } else {
            input.type = 'password';                    // Sembunyikan password
            icon.classList.remove('fa-eye-slash');
            icon.classList.add('fa-eye');                // Ganti icon jadi mata terbuka
        }
    }
</script>
@endsection
